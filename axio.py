...
class Axio(commands.Bot):
    def __init__(
        ...
        **kwargs,
        ...
        self.snipe_dict: dict[str, discord.Message] = {}
        self.edit_snipe_dict: dict[str, dict[str, discord.Message]] = {}
        self.copycat_ids = []
        self.animating = {
            "name": "",
            "frame_index": 0,
    ...
    async def setup_hook(self) -> None:
        for extension in self.initial_extensions:
            try:
                ...
                print(f"{F.LIGHTBLACK_EX}Error while loading extension {F.YELLOW}{extension}{F.LIGHTBLACK_EX}: {F.RED}{e}{F.LIGHTBLACK_EX}.\n")
                exit()
...
⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀⡀""", 2))
        ...
        print(f"{F.YELLOW}Axio{F.LIGHTBLACK_EX} is running on {F.YELLOW}{len(self.tokens)}{F.LIGHTBLACK_EX} account(s){F.RESET}\n")
        
        print(f"{F.LIGHTBLACK_EX}Logged in {F.YELLOW}{self.user.name} ({self.user.id}){F.RESET}")
        print(f"{F.LIGHTBLACK_EX}Prefixes: {F.YELLOW}{f'{F.LIGHTBLACK_EX}, {F.YELLOW}'.join(self.cfg['prefixes'])}{F.RESET}")
        ...
        print(f"{F.LIGHTBLACK_EX}Error Logging: {f'{F.GREEN}Enabled' if self.cfg['logging']['error_logging'] else f'{F.RED}Disabled'}")
        print(f"{F.LIGHTBLACK_EX}Nitro Sniper: {f'{F.GREEN}Enabled' if self.cfg['nitro_sniper']['enabled'] else f'{F.RED}Disabled'}{F.RESET}\n")
...
async def get_token_data(token: str) -> str | None:
...
async def start(token: str):
    ...
    if not os.path.isdir(cfg_path):
        ...
        with open(cfg_path + "/config.json", "w") as f:
            ...
            print(f"{F.RESET}[{F.YELLOW}{data['username']} {data['id']}{F.RESET}] {F.LIGHTBLACK_EX}Succesfully created config file {F.CYAN}{cfg_path}/config.json{F.LIGHTBLACK_EX}. If you want to modify it, you can apply the changes then use the {F.YELLOW}updatecfg{F.LIGHTBLACK_EX} command.")
    ...
    with open("./configs/tokens.json", "r") as f:
        data = json.load(f)
        _tokens = [val for sublist in data["tokens"].values() for val in (sublist if isinstance(sublist, list) else [sublist])]
...
def start_bots():
    with open("./configs/tokens.json", "r") as f:
        data = json.load(f)
        tokens = [val for sublist in data["tokens"].values() for val in (sublist if isinstance(sublist, list) else [sublist])]
